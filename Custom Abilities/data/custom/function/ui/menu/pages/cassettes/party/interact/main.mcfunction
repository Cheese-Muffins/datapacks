...
data modify storage minecraft:custom cassette.party.id set from block 0 -64 0 Items[0].components."minecraft:custom_data".ui_item.party_id
# already in party
execute if entity @s[tag=customCassette.PartyUser] run function custom:ui/menu/pages/cassettes/party/interact/zzz/0
# not in a party
execute unless entity @s[tag=customCassette.PartyUser] run function custom:ui/menu/pages/cassettes/party/interact/zzz/1 with storage minecraft:custom cassette.party
