...
$execute if score .Item$(number)_Count customUI.PurchaseItemCount matches $(required).. unless entity @s[tag=clickEvent] run function custom:ui/menu/pages/selection/inventory/zzz/0 {storage:"$(storage)",number:$(number),product:"$(product)"}
$execute unless score .Item$(number)_Count customUI.PurchaseItemCount matches $(required).. if entity @s[tag=clickEvent] run function custom:ui/menu/pages/selection/inventory/zzz/1 with storage minecraft:ui custom.selection.$(storage)