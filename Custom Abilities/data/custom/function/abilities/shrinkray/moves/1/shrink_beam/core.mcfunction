...
execute unless score @s customUniversal.FailReturn matches 1 run function custom:abilities/shrinkray/moves/1/shrink_beam/pass
execute if score @s customUniversal.FailReturn matches 1 run function custom:universal/error_messages/fail {number:1,unicode:"u50A0",move:"Shrink Beam"}