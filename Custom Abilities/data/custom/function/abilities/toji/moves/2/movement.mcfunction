...
$execute as @e[type=!#custom:not_mob,tag=!customGojo.LapisBlueVictim,distance=..3] unless entity @s[scores={customGojo.LapisBlueID=$(id)}] unless entity @s[tag=customSettings_Immunity] unless entity @s[gamemode=spectator] run function custom:abilities/toji/moves/2/zzz/1 {id:$(id)}
$execute as @e[type=!#custom:not_mob,tag=customGojo.LapisBlueVictim,scores={customGojo.LapisBlueID=$(id)}] unless entity @s[nbt={RootVehicle:1b}] run function custom:abilities/toji/moves/2/zzz/2