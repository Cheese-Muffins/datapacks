...
execute unless score @s customUniversal.FailReturn matches 1 run function custom:abilities/avatar/moves/2/pass
execute if score @s customUniversal.FailReturn matches 1 run function custom:universal/error_messages/fail {number:2,unicode:"u50A3",move:"Water Arrow"}