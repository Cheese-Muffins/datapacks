...
execute as @a[distance=..12] run function custom:abilities/the_world/moves/4/zzz/1
$execute at @s rotated ~180 0 as @p[tag=customAbility.TheWorld,tag=customTheWorld.FlurryCheck,scores={customTheWorld.FlurryID=$(id)}] run function custom:abilities/the_world/moves/4/zzz/2
execute at @s run function custom:abilities/the_world/moves/4/zzz/4
...
$execute as @n[type=minecraft:item_display,tag=aj.the_world.root,scores={customTheWorld.FlurryID=$(id)}] run function custom:abilities/the_world/moves/reset/all {user:"customAbility.TheWorld",objective:"customTheWorld.FlurryID",rig:"the_world"}