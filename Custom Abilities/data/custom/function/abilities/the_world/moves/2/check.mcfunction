...
tag @s remove temp
$execute if block ^ ^ ^$(speed) #custom:raycast_pass run function custom:abilities/the_world/moves/2/zzz/5
...
$execute if block ^ ^ ^$(speed) #custom:raycast_pass unless score @s customTheWorld.KnifeMovement matches 15.. if entity @s[tag=temp] positioned ^ ^ ^$(speed) run function custom:abilities/the_world/moves/2/check {speed:"$(speed)"}