...
execute unless entity @s[tag=customTheWorld.StoppingTime] run function custom:abilities/the_world/moves/2/zzz/0
execute if entity @s[tag=customTheWorld.StoppingTime] run data merge storage minecraft:custom {the_world:{knife_throw:{count:2,div_value:1}}}
