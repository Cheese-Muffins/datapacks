...
$execute as @e[type=!#custom:not_mob,tag=!customGojo_BluePickedUp,distance=..3] unless entity @s[scores={customUniversal_AttackID=$(id)}] run function custom:items/gojo/moves/2/pickup {id:"$(id)"}
$execute as @e[type=!#custom:not_mob,tag=customGojo_BluePickedUp,tag=customGojo_Blue$(id)] at @s at @n[type=minecraft:item_display,tag=aj.projectiles.root,scores={customUniversal_AttackID=$(id)}] run function custom:items/gojo/moves/2/mount
