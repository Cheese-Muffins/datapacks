...
execute at @s if score Lag_less Lagless matches 17 run playsound minecraft:timestop.tick player @a ~ ~ ~ 1
...
execute at @s run execute as @e[tag=!Stopped,type=!#interactables:directionusers,type=!#custom:not_mob,distance=..40] run data merge entity @s {Tags:["Stopped"],NoAI:1b,Motion:[0.0,0.0,0.0],NoGravity:1b}
...
execute if score Quickdraw Lagless matches 2 run execute at @a[tag=StoppingTime] run execute as @e[tag=!Stopped,type=!#interactables:directionusers,type=!#custom:not_mob,distance=..40] run data merge entity @s {Tags:["Stopped"],NoAI:1b,Motion:[0.0,0.0,0.0],NoGravity:1b}
