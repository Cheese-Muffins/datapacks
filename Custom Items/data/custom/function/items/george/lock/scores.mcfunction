...
execute unless score @s customHotBarSelection = @s customHotBarSelectionBefore run function custom:items/george/lock/update

execute unless entity @s[nbt={SelectedItem:{components:{"minecraft:custom_data":{moveTrigger:1b}}}}] run data modify storage minecraft:custom george.food.store set from entity @s SelectedItem
