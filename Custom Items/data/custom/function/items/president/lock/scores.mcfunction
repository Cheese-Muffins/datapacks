...
execute unless score @s customHotBarSelection = @s customHotBarSelectionBefore run function custom:items/president/lock/update

execute unless entity @s[nbt={SelectedItem:{components:{"minecraft:custom_data":{customMoveTrigger:1b}}}}] run data modify storage minecraft:custom president.food.store set from entity @s SelectedItem
...
execute unless data entity @s SelectedItem run item replace entity @p[tag=PresidentUser] weapon.mainhand with phantom_membrane[max_stack_size=1,item_name='{"text":" "}',hide_tooltip={},custom_model_data=1,custom_data={moveTrigger:1b},food={nutrition:0,saturation:0,eat_seconds:999999,can_always_eat:true}] 1
