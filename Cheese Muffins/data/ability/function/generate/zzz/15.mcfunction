...
$execute if score .item4Count uiAbility.ItemCount matches ..$(count) run function ability:generate/zzz/16 with storage minecraft:ability index.$(ability_id).price.$(target).request4
...
$function ability:generate/zzz/17 with storage minecraft:ability index.$(ability_id).price.$(target).request4