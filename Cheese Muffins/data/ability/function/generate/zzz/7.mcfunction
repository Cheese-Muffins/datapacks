...
$execute if score .item2Count uiAbility.ItemCount matches ..$(count) run function ability:generate/zzz/8 with storage minecraft:ability index.$(ability_id).price.$(target).request2
...
$function ability:generate/zzz/9 with storage minecraft:ability index.$(ability_id).price.$(target).request2