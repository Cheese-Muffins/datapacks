...
$execute if score .item1Count uiAbility.ItemCount matches ..$(count) run function ability:generate/zzz/4 with storage minecraft:ability index.$(ability_id).price.$(target).request1
...
$function ability:generate/zzz/5 with storage minecraft:ability index.$(ability_id).price.$(target).request1