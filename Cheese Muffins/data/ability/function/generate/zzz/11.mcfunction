...
$execute if score .item3Count uiAbility.ItemCount matches ..$(count) run function ability:generate/zzz/12 with storage minecraft:ability index.$(ability_id).price.$(target).request3
...
$function ability:generate/zzz/13 with storage minecraft:ability index.$(ability_id).price.$(target).request3