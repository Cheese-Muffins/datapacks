...
$function ui:purchase/generate/product {ability_id:$(ability_id),product_name:"skin5"}
$data modify storage minecraft:ui ability.choice merge from storage minecraft:ability index.$(ability_id)
execute store result storage minecraft:ui ability.choice.skin_count int 1 run scoreboard players get .skinCount uiAbility.Skin
scoreboard players reset .skinCount uiAbility.Skin