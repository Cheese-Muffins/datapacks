...
$execute if score @s cassette_cmd matches 10..99 run summon item ~ ~ ~ {Item:{id:"minecraft:carrot_on_a_stick",count:1,components:{"minecraft:custom_name":'{"text":"$(name)","color":"$(color)","italic":false}',"minecraft:lore":['{"text":"By: $(artist)","color":"gray","italic":false}'],"minecraft:custom_model_data":1$(id),"minecraft:custom_data":{cassette:redeem}}}}
execute if score @s cassette_cmd matches 100.. run function cassette:music/collect/100math with storage minecraft:cassette collect
execute if score @s cassette_cmd matches 100.. run function cassette:music/collect/100 with storage minecraft:cassette collect
