...
scoreboard players operation .2 cassette_total = @s cassette_total
execute store result storage minecraft:ui data.stats.score2 int 1 run scoreboard players get @s cassette_total
execute if items block 0 -62 0 container.0 minecraft:air run item replace block 0 -62 0 container.0 with minecraft:glass
item replace block 0 -62 0 container.1 with minecraft:player_head
data modify block 0 -62 0 Items[1].components."minecraft:profile".id set from entity @s UUID

schedule function cassette:music/redeem/leaderboard/schedule2 2t
