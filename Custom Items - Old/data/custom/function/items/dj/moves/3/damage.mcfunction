$damage @e[type=!#custom:not_mob,type=!minecraft:player,tag=!DJUser,sort=nearest,limit=1,distance=..$(range)] $(amount) minecraft:generic_kill by @p[tag=DJUser]
$damage @p[tag=!DJUser,distance=..$(range)] $(amount) minecraft:generic_kill by @p[tag=DJUser]
